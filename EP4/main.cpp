...
Digraph build_res_digraph(Digraph* d, Digraph* ad, std::vector<Arc>* ordering){
  ...
  for (tie(e_it, e_end) = boost::edges(rd); e_it!=e_end; e_it++){
    ...
    std::cout <<" res_capacity: " << res_capacity << std::endl;
...
void print_res_digraph(
  ...
  for(Arc a : *orig_ordering){
    Arc a_b, a_f;
    std::cout << "fwd_arc_idx: " << d[a].fwd_arc_idx << std::endl;
...
void edmonds_karp(FlowProblem &fp)
{
  ...
  while (true && t<1  )
  {
    ...
    print_res_digraph(fp.d, fp.a_d, &fp.arc_order, &fp.aug_arc_order);
//     std::tuple<
//         bool,
//         boost::optional<std::vector<Arc>>,
//         boost::optional<std::vector<Vertex>>>
